...
async def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            await manager.add_world(name, description=desc)
            ...
            print("\nVerfügbare Welten:")
            for idx, w in enumerate(manager.list_worlds(), start=1):
                print(f"{idx}) {w.name} (Saves: {w.save_count}, Größe: {w.size_mb} MB)")

            try:
                sel = int(input("Nummer der Welt auswählen: ").strip())
                chosen_world = manager.worlds[sel - 1]
