...
class World:
    ...
    def calculate_metadata(self):
        """Aktualisiert save_count, size_mb und last_played basierend auf den Save-Dateien."""
        ...
        for f in saves:
            try:
                ...
                # Falls ein Save während der Laufzeit verschoben/gelöscht wird
                continue
...
class WorldManager:
    ...
    async def load_worlds(self):
        if not os.path.exists(self.cfg.worlds_file):
            ...
            await self.save_worlds()  # erzeugt leere Datei []
            return
    ...
    async def save_worlds(self):
        ...
        logging.info("Worlds saved to JSON database.")
    ...
    async def add_world(self, name: str, description: str = ""):
        # Falls Ordner noch nicht existiert, wird er nicht automatisch angelegt
        # (du kannst das Verhalten hier anpassen)
        world = World(name, self.cfg.base_path, description=description)
    ...
    async def update_world_metadata(self, world: World):
        """Aktualisiert Metadaten einer Welt und schreibt sie in worlds.json."""
...
# --- Git Funktionen (async via to_thread) ---
...
async def start_game(use_experimental: bool, os_type: OSType):
    if os_type == OSType.WINDOWS:
        ...
        # os.system in Thread, da einfacher für Shell-URL
...
async def main():
    ...
    if not manager.worlds:
        # Wenn keine Welten existieren, füge zwei Beispielwelten hinzu
        await manager.add_world("AlphaWorld", description="Beispielwelt Alpha")
        await manager.add_world("BetaWorld", description="Beispielwelt Beta")
        logging.info("Initialized worlds database with demo worlds.")

    # Welt auswählen (hier: erste)
    chosen_world = manager.worlds[0]
    logging.info(f"Chosen world: {chosen_world}")

    # Optional: Git Pull im Welt-Ordner (falls es ein Repo ist)
