...
class Config:
    def __init__(self, os_type: OSType):
        ...
        # OS-spezifische Pfade und Prozessnamen
        if os_type == OSType.WINDOWS:
            self.base_path = os.path.join(
                os.path.expanduser("~"),
                "AppData", "Local", "FactoryGame", "Saved"
            )
            self.exe_name = "FactoryGameEGS.exe"
        elif os_type == OSType.LINUX:
            self.base_path = os.path.join(
                os.path.expanduser("~"),
                ".local", "share", "FactoryGame", "Saved"
            )
            # Unter Proton/Wine läuft Satisfactory als "FactoryGame" oder "wine"
            self.exe_name = "FactoryGame"
        elif os_type == OSType.MAC:
            self.base_path = os.path.join(
                os.path.expanduser("~"),
                "Library", "Application Support", "FactoryGame", "Saved"
            )
            # macOS: App-Bundle, Prozessname meist "FactoryGame"
            self.exe_name = "FactoryGame"
        else:
