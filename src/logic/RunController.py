...
class RunController:
    ...
    async def load_world(self, world: World):
    ...
    async def start_game(self, use_experimental: bool):
    ...
    async def wait_for_game_closed(self):
        started = False
        while True:
            running = await asyncio.to_thread(lambda: any(p.name() == self.executable_name for p in psutil.process_iter()))
            if running:
                if not started:
